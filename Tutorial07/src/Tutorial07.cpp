//--------------------------------------------------------------------------------------
// Archivo: Tutorial07.cpp
// 
// Nombre:  Bastidas Sanchez Diego Alejandro
//
...
CTime                               g_Time;
//Almacena matriz transpuesta de vista y proyección
Camera                              cam;            
...
InitDevice(){
    ...
    // Create the vertex shader AQUÍ DEBE IR EL INIT SHADER PROGRAM ???
    //hr = g_device.CreateVertexShader( pVSBlob->GetBufferPointer(), pVSBlob->GetBufferSize(), nullptr, &g_shaderProgram.m_VertexShader );
    //if( FAILED( hr ) ){    
    //    pVSBlob->Release();
    //    return hr;
    //}
    ...
    // Compile the pixel shader AQUÍ DEBE IR EL INIT SHADER PROGRAM ???
    ...
    //hr = g_device.CreatePixelShader( pPSBlob->GetBufferPointer(), pPSBlob->GetBufferSize(), nullptr, &g_shaderProgram.m_PixelShader );
    //pPSBlob->Release();
    //if (FAILED(hr)) {
    //    return hr;
    //}
    ...
    //Preguntarle a ChatGPT, UNA CLASE QUE CARGA INFORMACIÓN DE MODELOS
    /*LD = g_ModelTexture*/
